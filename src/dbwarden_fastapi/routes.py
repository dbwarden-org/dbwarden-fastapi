...
from dbwarden_fastapi.runtime import check_startup, compute_pending_migrations
...
def _compute_pending_seeds(db_name: str | None) -> int:
    """Pending seed count, or 0 when dbwarden-seeds is not installed.

    Seeds live in the dbwarden-seeds plugin, an optional companion to this one.
    Reporting 0 when it is absent is correct: with no seed pipeline there is
    nothing to be behind on.
    """
    try:
        from dbwarden_seeds.engine import get_pending_seeds
    except ImportError:
        return 0
