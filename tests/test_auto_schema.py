...
class TestAutoSchema:
    def test_auto_schema_raises_import_error(self):
        from dbwarden_fastapi.auto_schema import auto_schema

        with (
            patch("dbwarden_fastapi.auto_schema._schemap_auto_schema", None),
            pytest.raises(ImportError, match="schemap is required"),
        ):
            auto_schema(int)

    def test_schema_config_raises_import_error(self):
        from dbwarden_fastapi.auto_schema import SchemaConfig

        with (
            patch("dbwarden_fastapi.auto_schema._SchemaConfig", None),
            pytest.raises(ImportError, match="schemap is required"),
        ):
            SchemaConfig()

    @patch("dbwarden_fastapi.auto_schema._schemap_auto_schema")
    @patch("dbwarden_fastapi.auto_schema.apply_meta")
    @patch("dbwarden_fastapi.auto_schema._infer_schema_config")
    @patch("dbwarden_fastapi.auto_schema._merge_dbwarden_into_schemas")
    ...
    @patch("dbwarden_fastapi.auto_schema.SchemaConfig")
    def test_infer_schema_config(self, mock_sc):
        ...
        from dbwarden_fastapi.auto_schema import _infer_schema_config
    ...
    def test_merge_dbwarden_into_schemas(self):
        ...
        from dbwarden_fastapi.auto_schema import _merge_dbwarden_into_schemas

        with (
            patch.dict("sys.modules", {"pydantic": MagicMock()}),
            patch("dbwarden_fastapi.auto_schema.issubclass", return_value=True),
        ):
    ...
    def test_merge_schema_attr_is_none(self):
        ...
        with patch("dbwarden_fastapi.auto_schema.issubclass", return_value=True):
    ...
    def test_all_exports(self):
        from dbwarden_fastapi.auto_schema import __all__
    ...
    def test_schema_config_proxy_with_schemap(self):
        from dbwarden_fastapi.auto_schema import _SchemaConfig
